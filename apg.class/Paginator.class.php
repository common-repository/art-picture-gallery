...
public function createLinks( $links, $list_class ) {
    ...
    $html       .= '<li class="' . $class . '"><a href="'.$this->_galerieUrl.$this->_pos.'limit=' . $this->_limit . '&pagpage=' . ( $this->_pagpage - 1 ) . '#target">&laquo;</a></li>';    
    }
    if ( $start > 1 ) {
        $html   .= '<li><a href="'.$this->_galerieUrl.$this->_pos.'limit=' . $this->_limit . '&pagpage=1#target">1</a></li>';
        $html   .= '<li class="disabled"><span>...</span></li>';
    }
    for ( $i = $start ; $i <= $end; $i++ ) {
        $class  = ( $this->_pagpage == $i ) ? "active" : "";
        $html   .= '<li class="' . $class . '"><a href="'.$this->_galerieUrl.$this->_pos.'limit=' . $this->_limit . '&pagpage=' . $i . '#target">' . $i . '</a></li>';
    }
    if ( $end < $last ) {
        $html   .= '<li class="disabled"><span>...</span></li>';
        $html   .= '<li><a href="'.$this->_galerieUrl.$this->_pos.'limit=' . $this->_limit . '&pagpage=' . $last . '">' . $last . '#target</a></li>';
    }
    $class      = ( $this->_pagpage == $last ) ? "disabled" : "";
    if($class == 'disabled'){
    $html       .= '<li class="' . $class . '"><a>&raquo;</a></li>';    
    }else{
    $html       .= '<li class="' . $class . '"><a href="'.$this->_galerieUrl.$this->_pos.'limit=' . $this->_limit . '&pagpage=' . ( $this->_pagpage + 1 ) . '#target">&raquo;</a></li>';    
    }
    $html       .= '</ul>';
    return $html;
}
